...
export default function LeaguePointRankingPage() {
  ...
  return <Box sx={{ maxWidth:720, mx:"auto", p:2, pb:8 }}>
    ...
    <Box sx={{ opacity:enabled ? 1 : .62, pointerEvents:enabled ? "auto" : "none" }}>
      ...
      <Stack direction="row" flexWrap="wrap">{([['singles','단식'],['doubles','복식'],['team','단체전']] as const).map(([key,label]) => <FormControlLabel key={key} control={<Checkbox checked={rules.matchPoints.eventTypes[key]} onChange={(e) => setRules({ ...rules, matchPoints:{ ...rules.matchPoints, eventTypes:{ ...rules.matchPoints.eventTypes,[key]:e.target.checked } } })} />} label={label} />)}{([['league','풀리그'],['group','조별리그'],['tournament','토너먼트']] as const).map(([key,label]) => <FormControlLabel key={key} control={<Checkbox checked={rules.matchPoints.formats[key]} onChange={(e) => setRules({ ...rules, matchPoints:{ ...rules.matchPoints, formats:{ ...rules.matchPoints.formats,[key]:e.target.checked } } })} />} label={label} />)}</Stack>
