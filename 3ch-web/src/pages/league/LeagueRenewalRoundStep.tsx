...
export default function LeagueRenewalRoundStep({ kind }: { kind: StepKind }) {
  ...
  const renderOptions = (round: RenewalRoundConfig, index: number) => {
    if (kind === "type") return <>
      ...
      {hasParticipatingClubs && (round.program === "DOUBLES" || round.program === "TEAM") && <Box sx={{ mt: 2 }}><Typography sx={{ fontWeight: 900, mb: 1 }}>{round.program === "TEAM" ? "팀 구성" : "복식 구성"}</Typography><RadioGroup row value={round.unitClubMode ?? "mixed"} onChange={(event) => updateRound(index, { unitClubMode: event.target.value as "same" | "mixed", teamAssignments: undefined, doublesAssignments: undefined, groupAssignments: undefined })}><FormControlLabel value="same" control={<Radio />} label="같은 클럽만" /><FormControlLabel value="mixed" control={<Radio />} label="섞어서" /></RadioGroup></Box>}
    </>;
    if (kind === "format") return <>
      ...
      {hasParticipatingClubs && round.format !== "LEAGUE" && <Box sx={{ mt: 2 }}><Typography sx={{ fontWeight: 900, mb: 1 }}>타클럽 편성</Typography><RadioGroup row value={round.crossClubGrouping ? "yes" : "no"} onChange={(event) => updateRound(index, { crossClubGrouping: event.target.value === "yes", crossClubOnlyMatches: event.target.value === "yes" ? round.crossClubOnlyMatches : false })}><FormControlLabel value="yes" control={<Radio />} label="예" /><FormControlLabel value="no" control={<Radio />} label="아니오" /></RadioGroup></Box>}
      {hasParticipatingClubs && round.format !== "TOURNAMENT" && (round.program === "SINGLES" || round.unitClubMode === "same") && (round.format === "LEAGUE" || round.crossClubGrouping) && <Box sx={{ mt: 2 }}><Typography sx={{ fontWeight: 900, mb: 1 }}>타클럽만 매칭</Typography><RadioGroup row value={round.crossClubOnlyMatches ? "yes" : "no"} onChange={(event) => updateRound(index, { crossClubOnlyMatches: event.target.value === "yes" })}><FormControlLabel value="yes" control={<Radio />} label="예" /><FormControlLabel value="no" control={<Radio />} label="아니오" /></RadioGroup></Box>}
    </>;
