...
import { Alert, Box, Button, Chip, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, LinearProgress, MenuItem, Paper, Stack, Step, StepLabel, Stepper, TextField, Tooltip, Typography } from "@mui/material";
...
export default function LeagueQuickResult() {
  ...
  const recognize = async () => {
    try {
      ...
      setProgress(100); setParticipants(result.participants); setMatches(result.matches); setResultGroup(0); setResultOpen(true);
  ...
  const save = async () => {
    try {
      ...
      const unique = participants.filter((item, index, all) => all.findIndex((other) => other.name === item.name && other.division === item.division) === index);
      ...
      let saved = 0;
      for (const recognized of matches) {
  ...
  return <Box sx={{ maxWidth: 720, mx: "auto", pb: 12 }}>
    ...
    <Dialog open={resultOpen} onClose={()=>{if(!(progress>0&&progress<100))setResultOpen(false)}} fullWidth maxWidth="lg" slotProps={{paper:{sx:{position:"relative",borderRadius:3}}}}><DialogTitle fontWeight={900}>AI 인식 결과</DialogTitle><DialogContent dividers><Typography sx={{mb:2,color:"#6B7280",fontSize:13,fontWeight:700}}>이름 → 부수 순서와 별표 이후 세트 스코어를 인식했습니다. 잘못 인식된 내용은 직접 수정해 주세요.</Typography>{expectedFiles>1&&<Stack direction="row" spacing={1} sx={{mb:2,overflowX:"auto"}}>{Array.from({length:expectedFiles},(_,i)=><Button key={i} variant={resultGroup===i?"contained":"outlined"} onClick={()=>setResultGroup(i)} sx={{minWidth:64}}>{i+1}조</Button>)}</Stack>}<Stack spacing={1.5}>{participants.map((p,index)=>({p,index})).filter(({p})=>p.imageIndex===resultGroup).map(({p,index})=><Box key={p.key} sx={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 80px",gap:1}}><TextField size="small" label="이름" value={p.name} onChange={(e)=>setParticipants(all=>all.map((x,j)=>j===index?{...x,name:e.target.value}:x))} helperText={p.member_id?"우리리그 회원":p.rosterIncomplete?"팀원 추후 등록 가능":"임시회원"}/><TextField size="small" label="부수" value={p.division} onChange={(e)=>setParticipants(all=>all.map((x,j)=>j===index?{...x,division:e.target.value}:x))}/></Box>)}<Typography sx={{pt:1}} fontWeight={900}>경기 결과 {matches.filter(m=>m.imageIndex===resultGroup).length}건</Typography>{matches.map((m,index)=>({m,index})).filter(({m})=>m.imageIndex===resultGroup).map(({m,index})=><Stack key={`${m.participantAKey}-${m.participantBKey}-${index}`} direction="row" spacing={1} alignItems="center"><Typography sx={{flex:1,minWidth:0}}>{participants.find(p=>p.key===m.participantAKey)?.name}</Typography><TextField size="small" type="number" value={m.scoreA} onChange={(e)=>setMatches(all=>all.map((x,j)=>j===index?{...x,scoreA:Number(e.target.value)}:x))} sx={{width:64}}/><Typography>:</Typography><TextField size="small" type="number" value={m.scoreB} onChange={(e)=>setMatches(all=>all.map((x,j)=>j===index?{...x,scoreB:Number(e.target.value)}:x))} sx={{width:64}}/><Typography sx={{flex:1,minWidth:0}}>{participants.find(p=>p.key===m.participantBKey)?.name}</Typography>{m.needsReview&&<Chip color="warning" size="small" label="확인"/>}</Stack>)}</Stack><Paper variant="outlined" sx={{mt:2,p:1.5,bgcolor:"#F8FAFC"}}><Stack direction="row" alignItems="center"><Typography sx={{flex:1}} fontWeight={800}>본선 토너먼트도 생성할까요?</Typography><Button onClick={()=>setTournamentOpen(true)}>예</Button></Stack></Paper></DialogContent><DialogActions><Button disabled={progress>0&&progress<100} onClick={()=>setResultOpen(false)}>취소</Button><Button variant="contained" disabled={!participants.length||progress>0&&progress<100} onClick={save}>대진표에 입력</Button></DialogActions>{progress>0&&progress<100&&<Box sx={{position:"absolute",inset:0,bgcolor:"rgba(255,255,255,.82)",display:"grid",placeItems:"center",zIndex:2}}><Paper elevation={8} sx={{width:"min(420px,80%)",p:2.5,borderRadius:3}}><Stack direction="row" justifyContent="space-between"><Typography fontWeight={900}>경기 결과 저장 중</Typography><Typography color="primary" fontWeight={900}>{progress}%</Typography></Stack><LinearProgress variant="determinate" value={progress} sx={{mt:1,height:10,borderRadius:99}}/></Paper></Box>}</Dialog>
