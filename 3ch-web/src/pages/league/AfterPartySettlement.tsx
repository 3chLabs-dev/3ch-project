...
const roundTotals = (entry: Settlement) => ({
  ...
  food: (entry.items ?? []).filter((item) => item.category === "common").reduce((sum, item) => sum + Number(item.amount || 0), 0),
  alcohol: (entry.items ?? []).filter((item) => item.category === "alcohol").reduce((sum, item) => sum + Number(item.amount || 0), 0),
