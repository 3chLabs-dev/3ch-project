...
export default function DemoDrawPage() {
  ...
  return <><Seo title="샘플 추첨" description="샘플 참가자와 상품으로 우리리그의 실시간 일괄 추첨 기능을 직접 체험해보세요." path="/demo/draw" /><Stack spacing={2.2} sx={{ pb: 3 }}><Stack direction="row" alignItems="center" spacing={1}><IconButton onClick={() => navigate("/draw")} size="small"><ArrowBackIcon/></IconButton><Box flex={1} minWidth={0}><Typography fontWeight={900} fontSize={20} noWrap>우리리그 추첨</Typography><Stack direction="row" alignItems="center" spacing={1} mt={0.2}><Typography variant="caption" color="text.secondary" fontWeight={700}>2026-01-01(목) 18:00</Typography>{!hasResult && <Chip label="추첨 대기 중" size="small" sx={{ height: 18, fontWeight: 700, bgcolor: "#FFF7E6", color: "#F59E0B", fontSize: 11 }}/>}</Stack></Box></Stack>
