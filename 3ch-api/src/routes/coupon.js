...
router.post("/coupons/redeem", requireAuth, async (req, res) => {
  ...
  try {
    ...
    let benefit;
    if(FEATURES[coupon.type]){await client.query(`INSERT INTO feature_credit_buckets(user_id,feature,source,initial_amount,remaining_amount,starts_at,expires_at,source_ref) VALUES($1,$2,'COUPON',$3,$3,NOW(),$4,$5)`,[userId,FEATURES[coupon.type],coupon.value,coupon.valid_until,`coupon:${coupon.id}:${userId}`]);benefit={feature:FEATURES[coupon.type],count:coupon.value,expiresAt:coupon.valid_until};}
    ...
    await client.query(`INSERT INTO coupon_redemptions (coupon_id,user_id,status,benefit,applied_at) VALUES ($1,$2,$3,$4::jsonb,$5)`,
      [coupon.id,userId,immediatelyApplied?"APPLIED":"AVAILABLE",JSON.stringify(benefit),immediatelyApplied?new Date():null]);
...
router.get("/coupons/available",requireAuth,async(req,res)=>{const userId=Number(req.user.sub),plan=String(req.query.plan||"");const result=await pool.query(`SELECT r.id,c.name,c.type,c.value,c.plan_code,c.valid_until FROM coupon_redemptions r JOIN coupons c ON c.id=r.coupon_id WHERE r.user_id=$1 AND r.status='AVAILABLE' AND c.is_active=true AND c.valid_until>NOW() AND c.type='PERCENT_DISCOUNT' AND c.plan_code=$2 ORDER BY r.redeemed_at`,[userId,plan]);res.json({ok:true,coupons:result.rows});});
...
router.post("/admin/coupons",requireAdmin,async(req,res)=>{ const {name,type,value,planCode,validFrom,validUntil,quantity=1,distributionType="SINGLE",customCode,maxRedemptions}=req.body||{}; const isOpen=distributionType==="OPEN"; const count=isOpen?1:Math.min(100,Math.max(1,Number(quantity)||1));
  if(!name||!TYPES.has(type)||!Number.isInteger(Number(value))||Number(value)<1||!validUntil||new Date(validUntil)<=new Date(validFrom||Date.now())||(type==="PERCENT_DISCOUNT"&&Number(value)>99)||(["FREE_MONTHS","PERCENT_DISCOUNT"].includes(type)&&!planCode)||(isOpen&&!normalizeCode(customCode))||(maxRedemptions!==null&&maxRedemptions!==undefined&&maxRedemptions!==""&&Number(maxRedemptions)<1)) return res.status(400).json({ok:false,error:"INVALID_COUPON",message:"필수값, 할인율(1~99%), 사용기간을 확인해 주세요."});
