...
function getProgramRuleLabel(rule?: string) {
...
function getRuleDescription(rule?: string | null) {
  if (rule?.includes("3전 2선승") || rule === "BEST_OF_3") {
    return "세 번의 경기 중 두 번을 먼저 이기면 승리합니다.";
  }
  if (rule?.includes("5전 3선승") || rule === "BEST_OF_5") {
    return "다섯 번의 경기 중 세 번을 먼저 이기면 승리합니다.";
  }
  if (rule?.includes("7전 4선승")) {
    return "일곱 번의 경기 중 네 번을 먼저 이기면 승리합니다.";
  }
  if (rule?.includes("3세트") || rule === "THREE_SET") {
    return "세 번의 경기를 모두 진행하며, 2 대 0인 경우에도 세 번째 경기를 진행합니다.";
  }
  ...
  return (
    ...
    >
      ...
      {localOrder.map((colPlayer, colIdx) => {
        ...
        return (
          <BracketScoreCell key={colIdx} match={m} isA={isA} leagueId={leagueId} rules={m?.match_rule ?? rules} winScore={winScore} canManage={canScore} landscape={landscape} rowIndex={rowIdx} colIndex={colIdx} totalRows={n} totalCols={n} onProgramMatchUpdate={onProgramMatchUpdate}/>
        );
      })}
...
export default function LeagueBracket() {
  ...
  const hasGeneratedUnitRoster = generatedProgramMatchesAll.some((match) => {
    const unitMatch = match as LeagueMatch & {
      participant_a_roster?: string[];
      participant_b_roster?: string[];
    };
    return (unitMatch.participant_a_roster?.length ?? 0) > 1
  ...
  const programTeamParticipants = useMemo(() => {
    ...
    const unitSourceMatches = hasGeneratedUnitRoster ? generatedProgramMatchesAll : programMatchesAll;
    unitSourceMatches.forEach((match) => {
      ...
      if (isProgramMode && programOption) {
        ...
        const updateRound = <T extends { groupAssignments?: Array<Array<{ name: string; level: number; sourceGroupId?: string | null }>>; groupSizes?: number[]; participantOrder?: string[] }>(round: T): T => {
          ...
          return {
            ...
            groupSizes: groupIndex >= 0 && round.groupSizes ? round.groupSizes.map((size, index) => index === groupIndex ? size + 1 : size) : round.groupSizes,
        ...
        const nextMatches = generateProgramRoundMatches(id, nextProgram, [...rawParticipants, bot], programRound, programSourceMatches);
        storeProgramOption(id, nextProgram);
        await saveLeagueProgram({ leagueId: id, program: nextProgram }).unwrap();
        await syncProgramMatches({ leagueId: id, matches: nextMatches }).unwrap();
      }
            ...
            title={(
              <Box sx={{ py: 0.5 }}>
                <Typography sx={{ fontSize: 12, fontWeight: 800, color: "inherit" }}>
                  {getProgramRuleLabel(currentRule ?? "")}
                </Typography>
                <Typography sx={{ mt: 0.25, fontSize: 11, lineHeight: 1.5, color: "inherit" }}>
