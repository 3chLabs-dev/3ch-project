...
type LeagueDrawStatus = "not_started" | "scheduled" | "completed";

const LEAGUE_DRAW_STATUS_OPTIONS: Array<{ value: LeagueDrawStatus; label: string }> = [
  { value: "not_started", label: "추첨 미진행" },
  { value: "scheduled", label: "추첨 예정" },
  { value: "completed", label: "추첨 완료" },
];
...
export default function DrawMain() {
  ...
  const [leagueStatusFilters, setLeagueStatusFilters] = useState<LeagueDrawStatus[]>([]);
  ...
  return (
    <Stack spacing={2.2}>
      ...
      <SectionHeader
        ...
        filterActive={!!(leagueFilterStart || leagueFilterEnd || leagueStatusFilters.length)}
      />
      {!isLoggedIn ? (
        ...
        <Stack spacing={1}>
          {filteredLeagueSources.map((item) => (
            <LeagueResultCard
              ...
              canCreate={canCreate}
              navigate={navigate}
              statusFilters={leagueStatusFilters}
            />
          ))}
        </Stack>
      ) : (
      ...
      <DrawLeagueFilterDialog
        ...
        statusFilters={leagueStatusFilters}
        onApply={(start, end, statuses) => {
          setLeagueFilterStart(start);
          setLeagueFilterEnd(end);
          setLeagueStatusFilters(statuses);
        }}
      />
...
function LeagueResultCard({ item, canCreate, navigate, statusFilters }: {
  item: { id: string; name: string; league_code?: string; title?: string; type: string; start_date: string; };
  canCreate: boolean;
  navigate: (path: string) => void;
  statusFilters: LeagueDrawStatus[];
}) {
  ...
  const status: LeagueDrawStatus = !latestDraw
    ? "not_started"
    : Number(latestDraw.total_quantity) > 0
...
function DrawLeagueFilterDialog({
  ...
  endDate: string;
  statusFilters: LeagueDrawStatus[];
  onApply: (start: string, end: string, statuses: LeagueDrawStatus[]) => void;
}) {
  const startRef = useRef<HTMLInputElement | null>(null);
  const endRef = useRef<HTMLInputElement | null>(null);
  const [localStart, setLocalStart] = useState(startDate);
  const [localEnd, setLocalEnd] = useState(endDate);
  const [localStatuses, setLocalStatuses] = useState<LeagueDrawStatus[]>(statusFilters);
  ...
  const handleApply = () => {
    onApply(localStart, localEnd, localStatuses);
    onClose();
  };
    ...
    >
      <DialogTitle sx={{ pb: 1 }}>
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Typography sx={{ fontWeight: 900, fontSize: 18 }}>추첨 필터</Typography>
      ...
      <DialogContent sx={{ pt: 2 }}>
        <Stack spacing={2}>
          ...
          <Stack spacing={1}>
            <Typography sx={{ fontWeight: 800, fontSize: 14, color: "#6B7280" }}>상태</Typography>
            <Stack direction="row" spacing={0.8} useFlexGap flexWrap="wrap">
              {LEAGUE_DRAW_STATUS_OPTIONS.map((option) => {
                const active = localStatuses.includes(option.value);
                return (
                  <Button
                    key={option.value}
                    sx={chipButtonSx(active)}
                    onClick={() => setLocalStatuses((current) => active
