...
export default function LeagueRenewalStep1BasicInfo() {
  ...
  return <Box sx={{ px: 2.5, pt: 2 }}>
    ...
    <Box sx={{ borderTop: "1px solid #D9DDE6" }}>
      ...
      <Box sx={rowSx}><Typography sx={{ fontWeight: 900 }}>참가자 수</Typography><TextField select value={participantCount} onChange={(event) => setParticipantCount(event.target.value === "" ? "" : Number(event.target.value))} sx={fieldSx}><MenuItem value=""><em>-선택-</em></MenuItem>{participantCounts.map((count) => <MenuItem key={count} value={count}>{count}명</MenuItem>)}</TextField></Box>
      <Box sx={rowSx}><Typography sx={{ fontWeight: 900 }}>탁구대 수</Typography><TextField select value={courtCount} onChange={(event) => setCourtCount(event.target.value === "" ? "" : Number(event.target.value))} sx={fieldSx}><MenuItem value=""><em>-선택-</em></MenuItem>{[1, 2, 3, 4, 5, 6, 8, 10, 12].map((count) => <MenuItem key={count} value={count}>{count}대</MenuItem>)}</TextField></Box>
    </Box>
