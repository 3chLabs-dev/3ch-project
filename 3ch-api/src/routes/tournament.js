...
async function hasPremiumSubscription(client, userId) {
  const result = await client.query(
    `SELECT 1 FROM subscriptions WHERE user_id = $1 AND plan = 'premium'
...
const createSchema = z.object({
  ...
  divisions: z.array(z.object({
    ...
    league_type: z.string().trim().max(40).nullable().optional(),
...
router.post('/tournaments', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const created = await client.query(
      `INSERT INTO tournaments (title, description, starts_at, ends_at, host_group_id, created_by_id, premium_visible)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [data.title, data.description ?? null, data.starts_at, data.ends_at ?? null,
    ...
    for (const [index, division] of data.divisions.entries()) {
      await client.query(
        ...
        [created.rows[0].id, division.name, index, division.league_type ?? null, division.format ?? null,
