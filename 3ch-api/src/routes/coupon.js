...
router.post("/coupons/redeem", requireAuth, async (req, res) => {
  ...
  try {
    ...
    } else benefit={percent:coupon.value,planCode:coupon.plan_code || null};
...
router.post("/admin/coupons",requireAdmin,async(req,res)=>{ const {name,type,value,planCode,validFrom,validUntil,quantity=1,distributionType="SINGLE",customCode,maxRedemptions}=req.body||{}; const isOpen=distributionType==="OPEN"; const count=isOpen?1:Math.min(100,Math.max(1,Number(quantity)||1));
  if(!name||!TYPES.has(type)||!Number.isInteger(Number(value))||Number(value)<1||!validUntil||new Date(validUntil)<=new Date(validFrom||Date.now())||(type==="PERCENT_DISCOUNT"&&Number(value)>99)||(isOpen&&!normalizeCode(customCode))||(maxRedemptions!==null&&maxRedemptions!==undefined&&maxRedemptions!==""&&Number(maxRedemptions)<1)) return res.status(400).json({ok:false,error:"INVALID_COUPON"});
