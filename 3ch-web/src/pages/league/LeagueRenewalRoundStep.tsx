...
const newRound = (id: number): RenewalRoundConfig => ({ id, expanded: true, program: null, format: null, option: null, matchRule: null, teamPlayerCount: null, teamMatchType: null, tournamentSeeding: "seed" });
...
export default function LeagueRenewalRoundStep({ kind }: { kind: StepKind }) {
  ...
  const renderOptions = (round: RenewalRoundConfig, index: number) => {
    ...
    if (kind === "format") return <>
      <FormControl fullWidth><RadioGroup value={round.format ?? ""} onChange={(event) => updateRound(index, { format: event.target.value as NonNullable<RenewalRoundConfig["format"]>, tournamentSeeding: "seed" })}><Stack spacing={1.5}><FormControlLabel value="LEAGUE" control={<Radio />} label="단일리그" sx={optionCardSx} /><FormControlLabel value="GROUP" control={<Radio />} label="조별리그" sx={optionCardSx} /><FormControlLabel value="TOURNAMENT" control={<Radio />} label="토너먼트" sx={optionCardSx} /></Stack></RadioGroup></FormControl>
